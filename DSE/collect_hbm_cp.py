"""
collect_hbm_cp.py -- gather the class-parallel streaming-gather sweep into one CSV.

Run AFTER scripts/sweep_hbm_gather_cp.tcl:
    cd C:/USC/fpga-hdc-hls
    python DSE/collect_hbm_cp.py

Reads proj_hbmcp_c<CP>/sol1/syn/report/hbm_gather_cp_top_csynth.rpt and writes
DSE/synth_results/hbm_cp_sweep.csv -- one row per channel count. This is the
class-parallel off-chip slice of the Pareto table Haoyang's DSE searches.
"""
import os
import re
import glob
...
def main():
    rows = []
    for proj in sorted(glob.glob(os.path.join(ROOT, "proj_hbmcp_c*"))):
        m = re.match(r"proj_hbmcp_c(\d+)", os.path.basename(proj))
        cp = int(m.group(1)) if m else None
        rpts = (glob.glob(os.path.join(proj, "sol1", "syn", "report", "hbm_gather_cp_top_csynth.rpt"))
    ...
    out_csv = os.path.join(ROOT, "DSE", "synth_results", "hbm_cp_sweep.csv")
